...
| desktop notification | PowerShell toast: BurntToast if installed, else the WinRT `ToastText02` notifier under PowerShell's own AppId | ⚠️ unit only (argv shape, and that title/body are not in the script). **No toast has been seen.** A **WSL** client took the Linux branch, which has no notification at all, so the toast was unreachable there by construction — measured on dell-2026 2026-09-02, fixed in 0.24.2 by routing WSL to the Windows seam through interop, as the clipboard already does |
| join / knock / nudge sounds | `System.Media.SoundPlayer` over a `.wav` from `%WINDIR%\Media`, else a per-kind `[console]::beep()` pattern | 🟡 CI resolves the table against the real `%WINDIR%\Media` and prints which branch a real runner takes. **No sound has been heard**, and which files exist on a given Windows build is still unknown. On **WSL** the measurement is in: a fresh distribution has no `paplay`, no `aplay` and no freedesktop theme, so every sound was SILENCE and only the terminal BEL fired (dell-2026, 2026-09-02). 0.24.2 falls through to the Windows `.wav` — probed as `/mnt/c/Windows/Media/…`, because that is what the path is called from inside WSL — and to the beep pattern when even that is absent |
...
| --- | --- | --- | --- |
| **mac host, LAN** | ✅ verified (release gates) | ⏳ in progress 2026-09-02 | 🟡 **partly RUN 2026-09-02** — knock + approval, mirror, F2, scrollback keys, `[Dana]:` attribution at the pane, `/c` invisible at the pane, reconnect-and-re-knock after a daemon restart. Open: Shift+Enter and the CSI-u newline (see the plan's S5), the toast and the sound (fixed, not yet re-measured) |
